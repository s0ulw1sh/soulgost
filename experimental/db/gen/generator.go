...
/*
examples:
	err := db.Q(&self.App.Db).Table("asdasd", "t").First([]db.Field{
			db.Field{"asdasd", &reply.Sid},
			db.Field{"ccc", db.Expr{"COUNT(a, b)"}},
		}, []db.Condition{
			db.Condition{"sid",   "=", pair.Sid},
			db.Condition{"brnid", "=", pair.Brnid},
			db.Condition{"zxzxc", "=", "asdasd"},
	})

	err = db.Q(&self.App.Db).Table("asdasd").Update([]db.Field{
		db.Field{"sid", reply.Sid},
		db.Field{"zxc", db.Expr{"COUNT(a, b)"}},
	}, []db.Condition{
		db.Condition{"sid",   "=", pair.Sid},
		db.Condition{"brnid", "=", pair.Brnid},
		db.Condition{"zxzxc", "=", "asdasd"},
	...
	db.Q(&self.App.Db).Table("asdasd").Count("sid", &cnt)

	db.Q(&self.App.Db).Table("test", "t").Insert([]db.Field{
		db.Field{"t.a", db.Expr{"SUM(a, b)"}},
		db.Field{"t.b", reply.B},
	})

	err := db.Q(&self.App.Db).Table("test", "t").Delete([]db.Condition{
		db.Condition{"a", "=", reply.A},
		db.Condition{"b", "<>", reply.B},
	})
*/
